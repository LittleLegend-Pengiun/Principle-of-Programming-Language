...
list_of_parameters: parameters (SEMI parameters)*;
parameters: id_list COLLON type_name;
id_list: NORMAL_ID (COMMA NORMAL_ID)*;
...
if_stmt: IF LB expression RB block_stmt 
		...
		(ELSE block_stmt)?;
for_in_stmt: FOREACH LB NORMAL_ID IN expression DOUBLE_DOT expression (BY expression)? RB block_stmt;
